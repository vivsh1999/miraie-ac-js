import { login, fetchHomes, fetchDeviceDetails } from '../core/api';
import { type AuthToken } from '../types/auth';
import { createMqttClient, publish } from '../core/mqtt';
import mqtt from 'mqtt';
import { PowerMode, HVACMode, FanMode, PresetMode, SwingMode, DisplayMode, ConvertiMode } from '../types/enums';

export class FluentDevice {
  private status: any;

  constructor(private session: Session, public data: any, private details: any) {
  ...
  async turnOn(): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ ps: 'on' }));
    return this;
  }

  async turnOff(): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ ps: 'off' }));
    return this;
  }

  async setTemperature(temp: number): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ actmp: temp.toString() }));
    return this;
  }

  async setHvacMode(mode: HVACMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ acmd: mode.toString() }));
    return this;
  }
  
  async setFanMode(mode: FanMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ acfs: mode.toString() }));
    return this;
  }

  async setPresetMode(mode: PresetMode): Promise<this> {
      let payload: Record<string, any> = {};
      if (mode === PresetMode.NONE) { payload = { acem: "off", acpm: "off", acec: "off", cnv: 0 }; }
      else if (mode === PresetMode.ECO) { payload = { acem: "on", acpm: "off", acec: "off", actmp: 26.0, cnv: 0 }; }
      else if (mode === PresetMode.BOOST) { payload = { acem: "off", acpm: "on", acec: "off", cnv: 0 }; }
      else if (mode === PresetMode.CLEAN) { payload = { acem: "off", acpm: "off", acec: "on", cnv: 0 }; }
  ...
  async setVSwingMode(mode: SwingMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ acvs: mode }));
    return this;
  }

  async setHSwingMode(mode: SwingMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ achs: mode }));
    return this;
  }

  async setDisplayMode(mode: DisplayMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ acdc: mode.toString() }));
    return this;
  }

  async setConvertiMode(mode: ConvertiMode): Promise<this> {
    await publish(this.session.getMqttClient(), this.getControlTopic(), this.buildPayload({ acem: "off", acpm: "off", cnv: mode }));
    return this;
  }
}
...
export const createSession = async (credentials: {username: string, password: string}): Promise<Session> => {
